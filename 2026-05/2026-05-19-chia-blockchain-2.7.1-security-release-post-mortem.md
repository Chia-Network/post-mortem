# chia blockchain 2.7.1 ΓÇö security release retrospective
...
### Peer connections ΓÇö handshake and WebSocket framing
...
### Mempool ΓÇö duplicate transaction validation race

When multiple peers delivered the same spend at once, the ΓÇ£already seenΓÇ¥ check and expensive pre validation were not atomic, so concurrent workers could repeat full validation for the same transaction and churn the bounded seen cache. 2.7.1 marks a spend as in flight before pre validation, treats in flight as already including, and clears the in flight marker in a finally block so known invalid bundles stay in the seen cache without revalidation storms.

### Full node stability ΓÇö asserts and sync state
...
### Resource exhaustion ΓÇö stores, caches, and parsing
